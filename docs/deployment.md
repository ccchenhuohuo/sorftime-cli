...
**3. 上游存在 IP 限制能力，但当前账号看起来没开。** 错误码 `400` 的含义是「未认证的 IP」，
说明 Sorftime 侧有这个机制。不过 2026-09-03 实测：约 65 次调用全部从一个海外 VPS 出口
IP 发出，无一次 `400`。若账号配了公司 IP 白名单，一个海外地址几乎不可能在名单里，
因此判断当前没有生效的白名单。

残留风险是它**将来可能被开启**——那时人手一份的模式会集体失效。52 个端点里没有账号设置类
接口，这项只能在 Sorftime 控制台查看和变更，API 侧看不到。分发前去后台确认一次即可。
